...
server. If they are not available in this session, the server is not
registered — run `./setup.sh`, then ask the user to restart Claude Code so the
tools load. Do not call `open_canvas()` until the tools exist.
